...
public interface NotesMapper {
    ...
    @Update("UPDATE notes SET note = #{note}, content = #{content},state=#{state} WHERE id = #{id}")
    int update(@Param("note") String note, @Param("content") String content,@Param("state") String state, @Param("id") long id);
